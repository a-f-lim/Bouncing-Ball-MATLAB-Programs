...
function BB_bifurcation_phase_BWD(v,theta_0,e,Amps,T)
    T = T / 1000;
    A_i = Amps(1) / 1000;
    A_f = Amps(end) / 1000;
    dt = 0.000025;
    ...
    %iterating the thing over amplitudes backwards:
    ...
    figure(4), hold on
    scatter(horizontals, verticals, 1, "blue", "filled")
    grid on; axis([A_i, A_f, -pi-eps, pi+eps]);
    title('Bouncing Ball Bifurcation Phase Diagram')
    xlabel("Amplitude (m)")
    ylabel("Impact Phase")
end